...
import type { IAppMenuItems } from "./types";
...
onBeforeMount(() => {
  ...
  isActiveMenu.value =
    activeItem.value === itemKey.value || activeItem
      ? String(activeItem.value).startsWith(itemKey.value + "-")
      : false;
});
...
watch(
  ...
  (newVal) => {
    isActiveMenu.value =
      ...
      String(newVal).startsWith(itemKey.value + "-");
  }
);
...
const checkActiveRoute = (item: IAppMenuItems) => {
